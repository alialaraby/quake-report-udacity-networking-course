...
public final class QueryUtils {
    ...
    public static ArrayList<Quakes> fetchEarthquakeData(String requestUrl) {


        //this code is to force the the background thread to delay for 2 seconds
        //so that we can see the simulation of a slow connection app behavior
        try {
            Thread.sleep(2000);
    ...
    public static ArrayList<Quakes> extractEarthquakes(String response) {
        ...
        try {
            ...
            for (int i=0; i<rootArray.length(); i++){
                ...
                JSONObject jsonObjectFromRootArray = rootArray.getJSONObject(i);
                JSONObject targetJSONObject = jsonObjectFromRootArray.getJSONObject("properties");
                Double magnitude = targetJSONObject.getDouble("mag");
                String location = targetJSONObject.getString("place");
                String url = targetJSONObject.getString("url");
                Long date = targetJSONObject.getLong("time");
                earthquakes.add(new Quakes(magnitude, getPrimaryLocation(location)
                        , getLocationOffset(location)
                        , QueryUtils.dateFormatter(date)
                        , QueryUtils.timeFormatter(date)
                        , url));
            }
    ...
    //method to convert a string to url
    public static URL createUrl(String response){
    ...
    public static String httpMakeConnection(URL url) throws IOException {
    ...
    private static String readFromStram(InputStream inputStream){
    ...
    public static String dateFormatter(long timeMillis){
    ...
    public static String timeFormatter(long timeMillis){
    ...
    public static String getPrimaryLocation(String location){

        if (location.contains("of")){

            String stringParts[] = location.split("of");
            String primaryLocation = stringParts[1];
    ...
    public static String getLocationOffset(String location){

        if (location.contains("of")){

            String stringParts[] = location.split("of");
            String locationOffset = stringParts[0];

            return locationOffset + "of" ;
        }else
            return " ";
